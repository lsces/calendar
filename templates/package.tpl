					...
					{foreach from=$week item=day}
						...
						<tr>
							<td class="calitems {if $day.day eq $navigation.display_focus_date} current{/if}{if $day.day eq $navigation.today} highlight{/if} {cycle values="odd,even"}">
								{* This branch only ever runs when $viewMode is 'weeklist' - a focus_month gate
								   ...
								   own check below already applies) - the `|| $viewMode eq "week"` copied from
								   that branch was dead code (never true here), silently hiding real items on
								   any day that rolled into an adjacent month. Found + fixed 2026-08-26. *}
