...
{* New, package_page.php-only nav - deliberately a separate file from
   ...
   Layout/height tidying (themes/css/config.css's .bitnav rule, added same
   ...
   first pass used - .bitnav's flex + justify-content:space-between spaces
   the three groups below directly, no float/clear classes needed. *}
<div class="bitnav">
	<ul class="pagination">
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.before.year}"  title="{$navigation.before.year|bit_long_date}"><span class="bitnav-arrow">&laquo;</span> {tr}Year{/tr}</a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.before.month}" title="{$navigation.before.month|bit_long_date}"><span class="bitnav-arrow">&laquo;</span> {tr}Month{/tr}</a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.before.week}"  title="{$navigation.before.week|bit_long_date}"><span class="bitnav-arrow">&laquo;</span> {tr}Week{/tr}</a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.before.day}"   title="{$navigation.before.day|bit_long_date}"><span class="bitnav-arrow">&laquo;</span> {tr}Day{/tr}</a></li>
	</ul>
	...
	<ul class="pagination">
		...
		<li class="bitnav-picker">
			{* Plain form as the li's direct child, no <span> wrapper - a <span>
			   ...
			   instead of anchored under the input). .bitnav-picker below gives
	...
	<ul class="pagination">
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.after.day}"   title="{$navigation.after.day|bit_long_date}">{tr}Day{/tr} <span class="bitnav-arrow">&raquo;</span></a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.after.week}"  title="{$navigation.after.week|bit_long_date}">{tr}Week{/tr} <span class="bitnav-arrow">&raquo;</span></a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.after.month}" title="{$navigation.after.month|bit_long_date}">{tr}Month{/tr} <span class="bitnav-arrow">&raquo;</span></a></li>
		<li><a href="{$baseCalendarUrl}&amp;todate={$navigation.after.year}"  title="{$navigation.after.year|bit_long_date}">{tr}Year{/tr} <span class="bitnav-arrow">&raquo;</span></a></li>
	</ul>
</div>
{/strip}
